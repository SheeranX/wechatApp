...
let flagMonth = currentDay.getMonth() + 1;
let flagDay = currentDay.getDate();
...
const conf = {
  ...
  calculateDays(year, month, curDate) {//计算显示的天数
    let days = [];
    let day;
    let selectMonth;
    let selectYear;
  ...
  tapDayItem(e) {
    ...
    if (this.config.type === 'timearea') {
      ...
      if (month == curMonth && days[idx].day < day)
      return;
      this.setData({
        [prevKey]: false,
        [key]: true,
...
export default (config = {}) => {
  ...
  self.getDate = conf.getDate.bind(self);
